    ...
    return getUserKundaliCall(netId).then((response) => {
        ...
        const formattedStats = stats.map(stat => {
            return {
                netId: stat.net_idd,
                name: stat.namee,
                contactNumber: stat.contact_numberr,
                emailID: stat.email_idd,
                dob: stat.date_of_birthh,
                roleID: stat.role_idd,
                roleName: stat.role_namee,
                roleDescription: stat.role_descriptionn,
                facilitiesUsed: stat.facilities_usedd,
                slotsBooked: stat.slots_bookedd,
                eventsBooked: stat.events_bookedd,
                ticketsBooked: stat.tickets_bookedd,
                equipmentBooked: stat.equipment_countt,
                equipmentRentCost: stat.total_equipment_rentt,
                totalTicketCost: stat.total_tickets_costt,
                totalRentPaid: stat.total_rent_paidd,
                netId2: stat.net_id22,
                startTime: stat.start_timee,
                endTime: stat.end_timee,
                slotId: stat.slot_idd,
                bookingDate: stat.booking_datee,
                facilityId: stat.facility_idd,
                facilityName: stat.facility_namee,
                slotDate: stat.slot_datee     
            }
        });
        return {
